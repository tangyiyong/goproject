...
func RegHttpMsgHandler() {
	//! 任务的消息处理
	http.HandleFunc("/get_task_data", mainlogic.Hand_GetTaskData)                //! 玩家请求全部的任务信息
	http.HandleFunc("/receive_task", mainlogic.Hand_RecvTaskAward)               //! 玩家请求领取完成任务奖励
	http.HandleFunc("/receive_taskscore", mainlogic.Hand_RecvTaskScoreAward)     //! 玩家请求领取任务积分宝箱奖励
	http.HandleFunc("/receive_achievement", mainlogic.Hand_RecvAchievementAward) //! 玩家请求领取成就奖励

	//玩家登录离开创建角色处理
	http.HandleFunc("/user_login_game", mainlogic.Hand_LoginGame)         //玩家登录游戏服
	http.HandleFunc("/user_enter_game", mainlogic.Hand_EnterGame)         //玩家进入游戏服
	http.HandleFunc("/user_leave_game", mainlogic.Hand_LeaveGame)         //玩家离开游戏服
	http.HandleFunc("/create_new_player", mainlogic.Hand_CreatePlayer)    //玩家创建角色
	http.HandleFunc("/query_server_time", mainlogic.Hand_QueryServerTime) //玩家查询服务器时间

	//! 签到的消息处理
	http.HandleFunc("/get_sign_data", mainlogic.Hand_GetSignData) //! 玩家请求获取签到信息
	...
	//! 神将商店的消息处理
	http.HandleFunc("/get_store", mainlogic.Hand_GetStoreData)             //! 玩家请求商店信息
	http.HandleFunc("/get_all_store_data", mainlogic.Hand_GetAllStoreData) //! 玩家请求所有商店信息
	http.HandleFunc("/refresh_store", mainlogic.Hand_RefreshStore)         //! 玩家请求刷新神将商店
	http.HandleFunc("/store_buy", mainlogic.Hand_Store_Buy)                //! 玩家请求购买神将商店商品
	...
	http.HandleFunc("/get_goods_buy_times", mainlogic.Hand_GetMallBuyTimes) //! 玩家请求道具商城商品购买次数
	...
	http.HandleFunc("/get_one_buy_times", mainlogic.Hand_GetGoodsBuyTimes)  //! 玩家请求单个商品购买次数
	...
	//! 三国志消息
	http.HandleFunc("/get_sanguozhi_data", mainlogic.Hand_SanGuoZhiData) //! 玩家请求获取三国志信息
	...
	http.HandleFunc("/arena_battle", mainlogic.Hand_ArenaBattle)                               //! 多次挑战竞技场
	...
	http.HandleFunc("/rob_gem_onekey", mainlogic.Hand_RobGemOnekey)      //! 玩家请求一键夺宝
	...
	http.HandleFunc("/composed_gem", mainlogic.Hand_ComposedGem)         //! 玩家请求合成宝物
	...
	http.HandleFunc("/query_activity_login", mainlogic.Hand_QueryLoginData)                           //! 查询累计登录活动信息
	http.HandleFunc("/query_activity_firstrecharge", mainlogic.Hand_QueryFirstChargeData)             //! 查询首冲活动信息
	...
	http.HandleFunc("/one_key_hunt_award", mainlogic.Hand_OneKeyReceiveHuntAward) //! 玩家请求一键领取所有巡回奖励
	...
	http.HandleFunc("/onekey_group_score_award", mainlogic.Hand_OneKeyReceiveGroupScoreAward) //! 玩家请求一键领取团购奖励
	...
	http.HandleFunc("/festival_discount_sale", mainlogic.Hand_BuyFestivalSaleItem)    //! 玩家请求购买欢庆佳节半价限购
	...
	http.HandleFunc("/onekey_award_center", mainlogic.Hand_RecvAwardCenterAwardOneyKey)
	...
	http.HandleFunc("/leave_guild", mainlogic.Hand_LeaveGuild)
	...
	http.HandleFunc("/change_guild_role", mainlogic.Hand_ChangeMemberPose)
	...
	http.HandleFunc("/cancellation_guild_apply", mainlogic.Hand_CancelGuildApply)
	http.HandleFunc("/get_guild_data", mainlogic.Hand_GetGuildData)
	http.HandleFunc("/buy_guild_copy_action", mainlogic.Hand_BuyCopyBattleTimes)
	...
	http.HandleFunc("/unset_backhero", mainlogic.Hand_UnsetBackHero)      //玩家更换援军英雄
	...
	http.HandleFunc("/fashion_melt_value", mainlogic.Hand_FashionMeltValue) //时装熔炼
	http.HandleFunc("/fashion_melt_award", mainlogic.Hand_FashionMeltAward) //时装熔炼
	...
	http.HandleFunc("/test_pass_copy", mainlogic.Hand_TestPassCopy)     //! 测试直接通关副本
	...
	http.HandleFunc("/send_private_mail", mainlogic.Hand_SendPrivateMail) //! 玩家请求所有的邮件
	...
	http.HandleFunc("/recv_score_time_award", mainlogic.Hand_RecvScoreTimeAward)
	...
	http.HandleFunc("/recv_score_continue_award", mainlogic.Hand_RecvContinueWinAward)
	http.HandleFunc("/get_score_report_req", mainlogic.Hand_GetScoreBattleReport)
	...
	//月光集市
	http.HandleFunc("/act_moonlight_shop_get_info", mainlogic.Hand_GetMoonhopData)
	http.HandleFunc("/act_moonlight_shop_exchangetoken", mainlogic.Hand_MoonShop_Exchange)
	http.HandleFunc("/act_moonlight_shop_reducediscount", mainlogic.Hand_MoonShop_ReduceDiscount)
	http.HandleFunc("/act_moonlight_shop_refreshshop_buy", mainlogic.Hand_MoonShop_RefreshShop_Buy)
	http.HandleFunc("/act_moonlight_shop_refreshshop_auto", mainlogic.Hand_MoonShop_RefreshShop)
	http.HandleFunc("/act_moonlight_shop_buygoods", mainlogic.Hand_MoonShop_BuyGoods)
	http.HandleFunc("/act_moonlight_shop_getscoreaward", mainlogic.Hand_MoonShop_GetScoreAward)
	...
	//领取激活礼包
	http.HandleFunc("/recv_gift_code", mainlogic.Hand_RecvGiftCode)
