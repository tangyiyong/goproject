...
func (player *TPlayer) DB_SaveHeroAt(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex]}})
	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex]}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex]}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroLevelExp(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()},
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()},
...
func (player *TPlayer) DB_SaveHeroBreakLevel(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].BreakLevel}})
	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d.breaklevel", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].BreakLevel}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d.breaklevel", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].BreakLevel}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroGodLevel(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{godlvl: player.HeroMoudle.CurHeros[nIndex].GodLevel,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{godlvl: player.HeroMoudle.BackHeros[nIndex].GodLevel,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{godlvl: player.BagMoudle.HeroBag.Heros[nIndex].GodLevel,
...
func (player *TPlayer) DB_SaveHeroWakeLevel(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{WakelevelName: player.HeroMoudle.CurHeros[nIndex].WakeLevel,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{WakelevelName: player.HeroMoudle.BackHeros[nIndex].WakeLevel,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{WakelevelName: player.BagMoudle.HeroBag.Heros[nIndex].WakeLevel,
...
func (player *TPlayer) DB_SaveHeroWakeItem(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].WakeItem}})
	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d.wakeitem", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].WakeItem}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d.wakeitem", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].WakeItem}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroCulture(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].Cultures}})

		FieldName = fmt.Sprintf("curheros.%d.culturescost", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].CulturesCost}})

	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d.cultures", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].Cultures}})

		FieldName = fmt.Sprintf("backheros.%d.culturescost", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].CulturesCost}})

	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d.cultures", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].Cultures}})

		FieldName = fmt.Sprintf("herobag.heros.%d.culturescost", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].CulturesCost}})
...
func (player *TPlayer) DB_SaveHeroDestiny(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldLevelName: player.HeroMoudle.CurHeros[nIndex].DestinyState,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldLevelName: player.HeroMoudle.BackHeros[nIndex].DestinyState,
			FieldExpName: player.HeroMoudle.CurHeros[nIndex].DestinyTime}})
	} else if posType == POSTYPE_BAG {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldLevelName: player.BagMoudle.HeroBag.Heros[nIndex].DestinyState,
			FieldExpName: player.HeroMoudle.CurHeros[nIndex].DestinyTime}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroDiaoWenQuality(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].DiaoWenQuality}})
	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d.diaowenquality", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].DiaoWenQuality}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d.diaowenquality", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].DiaoWenQuality}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroXiLian(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{ptys: player.HeroMoudle.CurHeros[nIndex].DiaoWenPtys,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{ptys: player.HeroMoudle.BackHeros[nIndex].DiaoWenPtys,
			backs: player.HeroMoudle.CurHeros[nIndex].DiaoWenBack}})
	} else if posType == POSTYPE_BAG {
		ptys := fmt.Sprintf("herobag.heros.%d.diaowenptys", nIndex)
		backs := fmt.Sprintf("herobag.heros.%d.diaowenback", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{ptys: player.BagMoudle.HeroBag.Heros[nIndex].DiaoWenPtys,
			backs: player.HeroMoudle.CurHeros[nIndex].DiaoWenBack}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveHeroQuality(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurHeros[nIndex].Quality}})
	} else if posType == POSTYPE_BACK {
		FieldName := fmt.Sprintf("backheros.%d.quality", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.BackHeros[nIndex].Quality}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("herobag.heros.%d.quality", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.HeroBag.Heros[nIndex].Quality}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveEquipStrength(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.HeroMoudle.CurEquips[nIndex].StrengLevel}})
	} else if posType == POSTYPE_BAG {
		FieldName := fmt.Sprintf("equipbag.equips.%d.strenglevel", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{FieldName: player.BagMoudle.EquipBag.Equips[nIndex].StrengLevel}})
	}
	return true
}
...
func (player *TPlayer) DB_SaveEquipRefine(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{refinelvl: player.HeroMoudle.CurEquips[nIndex].RefineLevel,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{refinelvl: player.BagMoudle.EquipBag.Equips[nIndex].RefineLevel,
...
func (player *TPlayer) DB_SaveEquipStar(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{star: player.HeroMoudle.CurEquips[nIndex].Star,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{star: player.BagMoudle.EquipBag.Equips[nIndex].Star,
...
func (player *TPlayer) DB_SavePetLevel(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{levelName: player.HeroMoudle.CurPets[nIndex].Level,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{levelName: player.BagMoudle.PetBag.Pets[nIndex].Level,
...
func (player *TPlayer) DB_SavePetStar(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{starName: player.HeroMoudle.CurPets[nIndex].Star}})
	} else if posType == POSTYPE_BAG {
		starName := fmt.Sprintf("petbag.pets.%d.star", nIndex)
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{starName: player.BagMoudle.PetBag.Pets[nIndex].Star}})
	}
	return true
}
...
func (player *TPlayer) DB_SavePetGod(posType int, nIndex int) bool {
	if posType == POSTYPE_BATTLE {
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerHero", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{levelName: player.HeroMoudle.CurPets[nIndex].God,
		...
		mongodb.UpdateToDB(appconfig.GameDbName, "PlayerBag", bson.M{"_id": player.GetPlayerID()}, bson.M{"$set": bson.M{levelName: player.BagMoudle.PetBag.Pets[nIndex].God,
