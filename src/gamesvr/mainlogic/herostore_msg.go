...
//! 获取所有商店信息
func Hand_GetAllStoreInfo(w http.ResponseWriter, r *http.Request) {
	...
	//! 解析消息
	var req msg.MSG_GetAllStoreGoodInfo_Req
	...
	//! 创建返回消息
	var response msg.MSG_GetAllStoreGoodInfo_Ack
	...
	//! 结算免费次数
	player.StoreModule.CheckReset(time.Now().Unix())

	//! 神将
	//! 获取当前神将商店信息
	response.RetCode = msg.RE_SUCCESS
	for _, v := range player.StoreModule.ShopItemLst {
		good := msg.THeroStoreItemInfo{}
		good.ID = v.ID
		good.Status = v.Status
		response.Hero.GoodsInfoLst = append(response.Hero.GoodsInfoLst, good)
	}

	response.Hero.FreeCount = player.StoreModule.FreeRefreshCount

	refreshTime := player.StoreModule.FreeRefreshTime - time.Now().Unix()
	if refreshTime < 0 {
		refreshTime = 0
	}
	...
	response.Hero.RefreshCount = player.StoreModule.RefreshCount
	...
	refreshTime = player.StoreModule.AwakeFreeRefreshTime - time.Now().Unix()
	if refreshTime < 0 {
		refreshTime = 0
	}
	...
	refreshTime = player.StoreModule.PetFreeRefreshTime - time.Now().Unix()
	if refreshTime < 0 {
		refreshTime = 0
	}
...
//! 获取神将商店信息
func Hand_GetHeroStoreInfo(w http.ResponseWriter, r *http.Request) {
	gamelog.Info("message: %s", r.URL.String())
	...
	//! 解析消息
	var req msg.MSG_GetStoreGoodsInfo_Req
	err := json.Unmarshal(buffer, &req)
	if err != nil {
	...
	//! 创建返回消息
	var response msg.MSG_GetStoreGoodsInfo_Ack
	response.RetCode = msg.RE_UNKNOWN_ERR
	defer func() {
		b, _ := json.Marshal(response)
		w.Write(b)
	...
	if req.StoreType == gamedata.StoreType_Hero {
		...
		//! 获取当前神将商店信息
		response.RetCode = msg.RE_SUCCESS
		response.FreeCount = player.StoreModule.FreeRefreshCount
		...
		response.RefreshCount = player.StoreModule.RefreshCount
...
//! 刷新神将商店
func Hand_RefreshHeroStore(w http.ResponseWriter, r *http.Request) {
	gamelog.Info("message: %s", r.URL.String())
...
//! 购买商品
func Hand_HeroStore_Buy(w http.ResponseWriter, r *http.Request) {
	gamelog.Info("message: %s", r.URL.String())
	...
	//! 解析消息
	var req msg.MSG_BuyItem_Store_Req
	err := json.Unmarshal(buffer, &req)
	if err != nil {
		gamelog.Error("Hand_RefreshHeroStore Unmarshal fail. Error: %s", err.Error())
		return
	...
	//! 创建返回消息
	var response msg.MSG_BuyItem_Store_Ack
	response.RetCode = msg.RE_UNKNOWN_ERR
	defer func() {
		b, _ := json.Marshal(response)
		w.Write(b)
