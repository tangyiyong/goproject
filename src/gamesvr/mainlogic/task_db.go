...
//! 设置玩家任务进度
func (taskmodule *TTaskMoudle) DB_UpdatePlayerTask(taskID int, count int, status int) bool {
...
//! 增加玩家成就达成列表
func (taskmodule *TTaskMoudle) DB_AddAchievementCompleteLst(achievementID int) bool {
	return mongodb.UpdateToDB(appconfig.GameDbName, "PlayerTask", bson.M{"_id": taskmodule.PlayerID}, bson.M{"$push": bson.M{"achievedlist": achievementID}})
}

//! 设置玩家成就进度
func (taskmodule *TTaskMoudle) DB_UpdatePlayerAchievement(taskID int, count int, status int) bool {
...
//! 设置玩家任务积分
func (taskmodule *TTaskMoudle) DB_UpdatePlayerTaskScore(score int) bool {
	return mongodb.UpdateToDB(appconfig.GameDbName, "PlayerTask", bson.M{"_id": taskmodule.PlayerID}, bson.M{"$set": bson.M{
		"taskscore": score}})
}

//! 设置玩家任务积分宝箱领取状态
func (taskmodule *TTaskMoudle) DB_UpdatePlayerTaskScoreAwardStatus() bool {
	return mongodb.UpdateToDB(appconfig.GameDbName, "PlayerTask", bson.M{"_id": taskmodule.PlayerID}, bson.M{"$set": bson.M{
