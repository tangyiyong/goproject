...
func (self *TActivityModule) DB_UpdateSingleRecharge(index int, info TSingleRechargeRecord) {
	...
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{filedName: info}})
}

func (self *TActivityModule) DB_AddNewLoginActivity(activity TActivityLogin) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"login": activity}})
}

func (self *TActivityModule) DB_AddNewRechargeActivity(activity TActivityRecharge) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"recharge": activity}})
}

func (self *TActivityModule) DB_AddNewDiscountActivity(activity TActivityDiscountSale) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"discountsale": activity}})
}

func (self *TActivityModule) DB_AddNewSingleRechargeActivity(activity TActivitySingleRecharge) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"singlerecharge": activity}})
}

func (self *TActivityModule) DB_AddNewLimitDailyActivity(activity TActivityLimitDaily) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"limitdaily": activity}})
}

func (self *TActivityModule) DB_AddNewSevenDay(activity TActivitySevenDay) {
	mongodb.UpdateToDB(appconfig.GameDbName, "PlayerActivity", bson.M{"_id": self.PlayerID}, bson.M{"$push": bson.M{"sevenday": activity}})
}
