...
func (self *TSummonModule) SetPlayerPtr(playerid int32, player *TPlayer) {
...
func (self *TSummonModule) OnCreate(playerid int32) {
	//! 初始化信息
	self.Normal.SummonTime = utility.GetCurTime()
	self.Senior.SummonTime = utility.GetCurTime()

	self.IsFirst = true

	self.Normal.ResetTime = utility.GetTodayTime() + 24*60*60
