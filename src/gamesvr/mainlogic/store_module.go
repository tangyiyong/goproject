...
type TStoreModule struct {
	...
	HeroRefreshCount     int          //! 今天剩余可刷新次数
	HeroFreeRefreshCount int          //! 免费刷新次数
	HeroFreeRefreshTime  int32        //! 免费刷新时间
	HeroShopItemLst      []TStoreItem //! 当前商品

	AwakeRefreshCount     int //! 觉醒商店可刷新次数
	AwakeFreeRefreshCount int //! 觉醒商店
	AwakeFreeRefreshTime  int32
	AwakeShopItemLst      []TStoreItem //! 当前商品

	PetRefreshCount     int //! 战宠商店可刷新次数
	PetFreeRefreshCount int //! 战宠商店
	PetFreeRefreshTime  int32
...
func (storemodule *TStoreModule) OnCreate(playerid int32) {
	//! 创建伊始,给予玩家满次免费刷新
	storemodule.HeroFreeRefreshCount = gamedata.StoreFreeRefreshTimes
	storemodule.HeroRefreshCount = storemodule.GetPlayerRefreshCounts()
...
func (storemodule *TStoreModule) RefreshGoods(needType int) {
	if needType == gamedata.StoreType_Hero {
		if len(storemodule.HeroShopItemLst) > 0 {
			//! 清空原有商品信息
			storemodule.HeroShopItemLst = []TStoreItem{}
		}
		...
		for _, v := range goodsLst {
			...
			good.Status = 0
			storemodule.HeroShopItemLst = append(storemodule.HeroShopItemLst, good)
		}
...
func (storemodule *TStoreModule) CheckRefreshDemand(storeType int) (bool, int) {
	...
	if storeType == gamedata.StoreType_Hero {
		if storemodule.HeroFreeRefreshCount <= 0 {
			isFree = false
		}

		if storemodule.HeroRefreshCount <= 0 {
			gamelog.Error("Hand_RefreshHeroStore error: Not enough refresh times.")
			return false, msg.RE_NOT_ENOUGH_REFRESH_TIMES
		}
...
func (storemodule *TStoreModule) PaymentTerms(storeType int) (int, int) {
	...
	if storeType == gamedata.StoreType_Hero {
		if storemodule.HeroFreeRefreshCount > 0 {
			...
			//! 免费刷新次数减一
			storemodule.HeroFreeRefreshCount -= 1
			storemodule.HeroRefreshCount -= 1
			storemodule.DB_UpdateRefreshCount(gamedata.StoreType_Hero)
			if storemodule.HeroFreeRefreshTime == 0 {
				//! 设置刷新次数CD时间
				storemodule.HeroFreeRefreshTime = utility.GetCurTime() + int32(gamedata.StoreFreeRefreshAddTime)
			}
		...
		if storemodule.AwakeFreeRefreshCount > 0 {
			...
			if storemodule.AwakeFreeRefreshTime == 0 {
				//! 设置刷新次数CD时间
				storemodule.AwakeFreeRefreshTime = utility.GetCurTime() + int32(gamedata.StoreFreeRefreshAddTime)
			}
		...
		if storemodule.PetFreeRefreshCount > 0 {
			...
			if storemodule.PetFreeRefreshTime == 0 {
				//! 设置刷新次数CD时间
				storemodule.PetFreeRefreshTime = utility.GetCurTime() + int32(gamedata.StoreFreeRefreshAddTime)
			}
...
func (self *TStoreModule) RedTip() (bool, IntLst) {
	isRed := false
	redLst := IntLst{}
	if self.HeroFreeRefreshCount != 0 {
...
func (storemodule *TStoreModule) CheckGoodsStatus(index int, storeType int) (bool, int) {
	...
	var item *TStoreItem
	if storeType == gamedata.StoreType_Hero {
		item = &storemodule.HeroShopItemLst[index]
	} else if storeType == gamedata.StoreType_Awake {
...
func (storemodule *TStoreModule) PayGoods(id int, storeType int) {
	item := storemodule.HeroShopItemLst[id]

	//! 修改标记
	if storeType == gamedata.StoreType_Hero {
		storemodule.HeroShopItemLst[id].Status = 1
...
//! 刷新免费次数
func (storemodule *TStoreModule) CheckReset(now int32) {
	if utility.IsSameDay(storemodule.ResetDay) == false {
		storemodule.HeroRefreshCount = storemodule.GetPlayerRefreshCounts()
		storemodule.AwakeRefreshCount = storemodule.HeroRefreshCount
		storemodule.PetRefreshCount = storemodule.HeroRefreshCount
	...
	//! 免费次数不得大于最大免费次数限制
	if storemodule.HeroFreeRefreshCount >= gamedata.StoreFreeRefreshTimes {
		storemodule.HeroFreeRefreshTime = 0
	}
	...
	for {
		if now >= storemodule.HeroFreeRefreshTime && storemodule.HeroFreeRefreshTime != 0 {
			//! 免费次数加一
			storemodule.HeroFreeRefreshCount += 1
			...
			if storemodule.HeroFreeRefreshCount >= gamedata.StoreFreeRefreshTimes {
				...
				storemodule.HeroFreeRefreshTime = storemodule.HeroFreeRefreshTime + int32(gamedata.StoreFreeRefreshAddTime)
			}
	...
	for {
		if now >= storemodule.AwakeFreeRefreshTime && storemodule.AwakeFreeRefreshTime != 0 {
			...
			if storemodule.AwakeFreeRefreshCount >= gamedata.StoreFreeRefreshTimes {
				...
				storemodule.AwakeFreeRefreshTime = storemodule.AwakeFreeRefreshTime + int32(gamedata.StoreFreeRefreshAddTime)
			}
	...
	for {
		if now >= storemodule.PetFreeRefreshTime && storemodule.PetFreeRefreshTime != 0 {
			...
			if storemodule.PetFreeRefreshCount >= gamedata.StoreFreeRefreshTimes {
				...
				storemodule.PetFreeRefreshTime = storemodule.PetFreeRefreshTime + int32(gamedata.StoreFreeRefreshAddTime)
			}
