...
type MSG_BeachBaby_Info_Ack struct {
	...
	AutoRefreshTime int32
	Score           [2]int
	TotalScore      int
	RankAward       [2]int8 //! 排行奖励领取标记 0:表示今天，1:表示总榜
	FreeConchBit    int32   // 特定时间免费领取贝壳
}
...
func Hand_BeachBaby_Info(w http.ResponseWriter, r *http.Request) {
	...
	//! 检测当前是否有此活动
	isHandleTime := G_GlobalVariables.IsActivityTime(player.ActivityModule.BeachBaby.ActivityID)
	if isHandleTime == false {
		response.RetCode = msg.RE_ACTIVITY_NOT_OPEN
		return
	}

	player.ActivityModule.BeachBaby.Refresh_Auto()
	response.Goods = player.ActivityModule.BeachBaby.Goods
	response.AutoRefreshTime = player.ActivityModule.BeachBaby.AutoTime
	response.Score = player.ActivityModule.BeachBaby.Score
	response.TotalScore = player.ActivityModule.BeachBaby.TotalScore
	response.RankAward = player.ActivityModule.BeachBaby.RankAward
	response.FreeConchBit = int32(player.ActivityModule.BeachBaby.FreeConch)
...
func Hand_BeachBaby_OpenGoods(w http.ResponseWriter, r *http.Request) {
	...
	item, bGetItem := player.ActivityModule.BeachBaby.OpenGoods(req.Index)
