...
func Hand_GetAllTask(w http.ResponseWriter, r *http.Request) {
	...
	pPlayer.TaskMoudle.CheckReset()

	//! 获取数据
	for _, v := range pPlayer.TaskMoudle.TaskList {
		task := msg.TTaskInfo{}
...
func Hand_GetTaskAward(w http.ResponseWriter, r *http.Request) {
	...
	//! 检查任务完成情况
	ret, errcode := pPlayer.TaskMoudle.CheckPlayerTask(req.TaskID)
	if ret == false {
	...
	//! 发放奖励
	ret, itemLst := pPlayer.TaskMoudle.ReceiveTaskAward(req.TaskID)
	if ret == false {
...
func Hand_GetTaskScoreAward(w http.ResponseWriter, r *http.Request) {
	...
	//! 判断玩家积分领取资格
	ret, errcode := pPlayer.TaskMoudle.CheckTaskScore(req.ScoreAwardID)
	if ret == false {
		gamelog.Error("Hand_GetTaskScoreAward error : Score not enough player : %d  score: %d  ask: %d",
			req.PlayerID, pPlayer.TaskMoudle.TaskScore, req.ScoreAwardID)
		response.RetCode = errcode
		return
	}

	//! 发放积分奖励
	ret, itemLst := pPlayer.TaskMoudle.ReceiveTaskScoreAward(req.ScoreAwardID)
	if ret == false {
	...
	//! 改变领取标记
	pPlayer.TaskMoudle.ScoreAwardStatus.Add(req.ScoreAwardID)

	//! 当前领取状态更新到数据库
	go pPlayer.TaskMoudle.UpdatePlayerTaskScoreAwardStatus()

	response.RetCode = msg.RE_SUCCESS

	for _, v := range pPlayer.TaskMoudle.ScoreAwardID {
		var scoreaward msg.MSG_ScoreAward
		scoreaward.ScoreAwardID = v
		if pPlayer.TaskMoudle.ScoreAwardStatus.IsExist(v) >= 0 {
